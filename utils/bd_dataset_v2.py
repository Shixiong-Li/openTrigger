...
class prepro_cls_DatasetBD_v2(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        '''
        self.attack = attack
        self.blend_image_files_path = blend_image_files_path
    ...
    def prepro_backdoor(self):
        for selected_index in tqdm(self.original_index_array, desc="prepro_backdoor"):
            if self.poison_indicator[selected_index] == 1:
                ...
                if self.attack== 'OpenTrigger':
                    image_files = [f for f in os.listdir(self.blend_image_files_path) if os.path.isfile(os.path.join(self.blend_image_files_path, f))]
                    image_path = os.path.join(self.blend_image_files_path, random.choice(image_files))
                    img = Image.blend(img, Image.open(image_path), alpha=self.blend_ratio)
